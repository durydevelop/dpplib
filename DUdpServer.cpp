...
void DUdpSession::handle_request(const std::error_code& ec, std::size_t BytesRecvd)
{
...
void DUdpSession::handle_sent(const std::error_code& ec, std::size_t BytesSent) {
...
void DUdpServer::receive_session() {
...
void DUdpServer::handle_receive(shared_session session, const std::error_code& ec, std::size_t BytesRecvd) {
...
void DUdpServer::enqueue_response(shared_session const& session, size_t len) {
    socket_.async_send_to(
        asio::buffer(session->SendBuffer,len),
        session->RemoteEndpoint,
        strand_.wrap(bind(&DUdpSession::handle_sent,
            session, // keep-alive of buffer/endpoint
            std::placeholders::_1,
            std::placeholders::_2)));
}
