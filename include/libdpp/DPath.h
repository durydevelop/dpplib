...
namespace DPath
{
	...
	bool Copy_File(const fs::path &From, const fs::path &To, bool OverwriteExisting, bool SafeMode = false);
	...
	bool CopyDir(fs::path SourceDir, fs::path DestDir, bool FailIfExists, DCallback Callback = nullptr);
	//err::error_code MoveDir(fs::path SourceDir, fs::path DestDir, bool FailIfExists, DGlobalCallback Callback);
	bool MoveDir(fs::path SourceDir, fs::path DestDir, bool FailIfExists, DCallback Callback = nullptr);
	...
	int CountFiles(fs::path PathToScan, bool Recoursive, std::string NameContent, bool NameWholeWord, std::string ExtContent, bool ExtWholeWord, bool CaseSensitive, bool *StopRequest);
	int CountFiles(fs::path PathToScan, bool Recoursive, bool *StopRequest);
	int DeleteFiles(fs::path PathToSan, bool Recoursive, std::vector<std::string> *NameContentList, bool NameWholeWord, std::vector<std::string> *ExtContentList, bool ExtWholeWord, bool CaseSensitive, bool FindAll, bool *StopRequest);
